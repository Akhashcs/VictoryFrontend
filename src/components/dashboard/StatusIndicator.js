...
const StatusIndicator = ({ type, status, loading = false, onRefresh }) => {
  ...
  return (
    <div className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg border ${colorClasses[config.color]}`}>
      {/* Mobile: Colored dot only */}
      <div className={`w-3 h-3 rounded-full ${config.dotColor} sm:hidden`}></div>
